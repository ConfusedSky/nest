...
#define DISPATCH() goto loop
  ...
  INTERPRET_LOOP
  {
    CASE_CODE(LOAD_LOCAL_0) : CASE_CODE(LOAD_LOCAL_1) : CASE_CODE(LOAD_LOCAL_2) : CASE_CODE(LOAD_LOCAL_3) : CASE_CODE(LOAD_LOCAL_4) : CASE_CODE(LOAD_LOCAL_5) : CASE_CODE(LOAD_LOCAL_6) : CASE_CODE(LOAD_LOCAL_7) : CASE_CODE(LOAD_LOCAL_8) : PUSH(stackStart[instruction - CODE_LOAD_LOCAL_0]);
    DISPATCH();
    ...
    CASE_CODE(POP) : DROP();
    DISPATCH();
    CASE_CODE(NULL) : PUSH(NULL_VAL);
    DISPATCH();
    CASE_CODE(FALSE) : PUSH(FALSE_VAL);
    DISPATCH();
    CASE_CODE(TRUE) : PUSH(TRUE_VAL);
    DISPATCH();

    CASE_CODE(STORE_LOCAL) : stackStart[READ_BYTE()] = PEEK();
    DISPATCH();

    CASE_CODE(CONSTANT) : PUSH(fn->constants.data[READ_SHORT()]);
    DISPATCH();
    ...
    CASE_CODE(LOAD_MODULE_VAR) : PUSH(fn->module->variables.data[READ_SHORT()]);
    DISPATCH();

    CASE_CODE(STORE_MODULE_VAR) : fn->module->variables.data[READ_SHORT()] = PEEK();
    DISPATCH();
    ...
    CASE_CODE(CLOSE_UPVALUE) : // Close the upvalue for the local if we have one.
    ...
    CASE_CODE(CONSTRUCT) : ASSERT(IS_CLASS(stackStart[0]), "'this' should be a class.");
    stackStart[0] = wrenNewInstance(vm, AS_CLASS(stackStart[0]));
    DISPATCH();

    CASE_CODE(FOREIGN_CONSTRUCT) : ASSERT(IS_CLASS(stackStart[0]), "'this' should be a class.");
    ...
    CASE_CODE(METHOD_INSTANCE) : CASE_CODE(METHOD_STATIC) :
    {
    ...
    CASE_CODE(END) : // A CODE_END should always be preceded by a CODE_RETURN. If we get here,
...
#define VALIDATE_API_SLOT(vm, slot)                              \
  {                                                              \
...
{
  VALIDATE_API_SLOT(vm, slot);
  VALIDATE_API_SLOT(vm, classSlot);
...
{
  VALIDATE_API_SLOT(vm, listSlot);
  VALIDATE_API_SLOT(vm, elementSlot);
...
{
  VALIDATE_API_SLOT(vm, mapSlot);
  VALIDATE_API_SLOT(vm, keySlot);
...
void wrenGetMapValue(WrenVM *vm, int mapSlot, int keySlot, int valueSlot)
{
  VALIDATE_API_SLOT(vm, mapSlot);
  VALIDATE_API_SLOT(vm, keySlot);
  VALIDATE_API_SLOT(vm, valueSlot);
  ASSERT(IS_MAP(vm->apiStack[mapSlot]), "Slot must hold a map.");
